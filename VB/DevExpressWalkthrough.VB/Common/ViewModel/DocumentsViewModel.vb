...
Namespace Global.DevExpressWalkthrough.VB.Common.ViewModel
    ...
    Public MustInherit Class DocumentsViewModel(Of TModule As ModuleDescription(Of TModule), TUnitOfWork As IUnitOfWork)
        ...
        Protected Sub New(ByVal unitOfWorkFactory As IUnitOfWorkFactory(Of TUnitOfWork))
            ...
            Next
            Messenger.[Default].Register(Of DestroyOrphanedDocumentsMessage)(Me, Sub(ByVal x) DestroyOrphanedDocuments())
        End Sub
        ...
        Public Overridable Sub OnClosing(ByVal cancelEventArgs As CancelEventArgs)
            If GroupedDocumentManagerService IsNot Nothing AndAlso GroupedDocumentManagerService.Groups.Count() > 1 Then
                ...
                Dim message = New CloseAllMessage(cancelEventArgs, Function(ByVal vm)
                                                                       Dim activeVMs = activeGroup.Documents.[Select](Function(ByVal d) d.Content)
            ...
            End If
            Messenger.[Default].Send(New CloseAllMessage(cancelEventArgs, Function(ByVal vm) True))
        ...
        Public Function ShowCore(ByVal [module] As TModule) As IDocument
            If [module] Is Nothing OrElse DocumentManagerService Is Nothing Then
                Return Nothing
            End If
            Dim document As IDocument = DocumentManagerService.FindDocumentByIdOrCreate([module].DocumentType, Function(ByVal x) CreateDocument([module]))
            document.Show()
            Return document
        End Function
        ''' <summary>
        ...
        Public Sub PinPeekCollectionView(ByVal [module] As TModule)
            If WorkspaceDocumentManagerService Is Nothing Then
                Return
            End If
            Dim document As IDocument = WorkspaceDocumentManagerService.FindDocumentByIdOrCreate([module].DocumentType, Function(ByVal x) CreatePinnedPeekCollectionDocument([module]))
            document.Show()
        End Sub
        ''' <summary>
        ...
        Private Sub OnActiveDocumentChanged(ByVal sender As Object, ByVal e As ActiveDocumentChangedEventArgs)
            ...
            Else
                ...
                ActiveModule = Modules.FirstOrDefault(Function(ByVal m) m.DocumentType = CType(e.NewDocument.Id, String))
